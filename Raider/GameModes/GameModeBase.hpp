...
{
public:
    AbstractGameModeBase(const std::string BasePlaylist, bool bRespawnEnabled = false, int maxTeamSize = 1, bool bRegenEnabled = false, bool bAntiRejoin = false)
    {
    ...
    {
        auto PlayerId = static_cast<APlayerState*>(Controller->PlayerState)->PlayerID;
        LOG_INFO("({}) Initializing {} - {} that has just joined!", "GameModeBase", Controller->PlayerState->GetPlayerName().ToString(), std::to_string(PlayerId));
        ...
        if (this->bAntiRejoin == true && std::find(joinedPlayersIds.begin(), joinedPlayersIds.end(), PlayerId) != joinedPlayersIds.end())
        {
            LOG_INFO("Kicking {} because his PlayerId is in the joinedPlayerIds array.", Controller->PlayerState->GetPlayerName().ToString());
            KickController(Controller, L"You got kicked because your PlayerId is matching with the list of disconnected players, if you want to rejoin, change your username and rejoin !");
            return;
        }
